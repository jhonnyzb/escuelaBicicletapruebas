...
class Festivos
{
	...
	{
		return new Festivos($year);
	}
	
	private function __construct($ano='')
	{
		$this->hoy=date('d/m/Y');
	...
	public function toHumanDates()
	{
		$fechas = [];	
		foreach ($this->festivos[date('Y')] as $meses => $dias) 
		{
			foreach ($dias as $dia => $value) 
			{
				$fechas[] = Carbon::createFromFormat('Y-m-d', date('Y').'-'.$meses.'-'.$dia);
			}
		}
	...
	public function datesToString()
	{
		...
		foreach ($this->festivos[date('Y')] as $meses => $dias) 
		{
			foreach ($dias as $dia => $value) 
			{
				$fechas .= Carbon::createFromFormat('Y-m-d', date('Y').'-'.$meses.'-'.$dia)->toDateString().',';
			}
		}

		return substr($fechas, 0, -1);
	}
}